...
PUBLIC_LATEST="https://rathgwvfewasazxlpusx.supabase.co/storage/v1/object/public/public-viewer/snapshots/latest.json"
# force_publish 本体は main の lan_site_publish を使う
...
CACHE_OK=0
DONE_OK=0
[[ -f "$ROOT/logs/morning_bulk_races_${DAY}.pkl" ]] && CACHE_OK=1
[[ -f "$ROOT/logs/morning_bulk_done_${DAY}.flag" || -f "$ROOT/logs/morning_bulk_done_${DAY//-/}.flag" ]] && DONE_OK=1
...
print(f"cache_n={len(obj) if hasattr(obj,'__len__') else '?'} type={type(obj).__name__}")
PY
fi

section "3) public latest BEFORE"
"$ROOT/.venv/bin/python" - <<'PY' 2>/dev/null || python3 - <<'PY'
...
if [[ "$AUTO_STATE" != "active" ]]; then
  ...
  if [[ -x "$ROOT/server_deployment/race_day_start_hwm.sh" ]]; then
    YOKUMAKUN_ROOT="$ROOT" YOKUMAKUN_SERVER_AUTO_SERVICE="$SERVICE" \
      bash "$ROOT/server_deployment/race_day_start_hwm.sh" 2>&1 | tail -n 40 || true
  else
    sudo_run systemctl start "$SERVICE" 2>&1 | tail -n 20 || true
  fi
  sleep 2
  log "automation_now=$(systemctl is-active "$SERVICE" 2>/dev/null || echo missing)"
  systemctl --no-pager -l status "$SERVICE" 2>&1 | sed -n '1,22p' || true
else
...
if [[ -f "$ROOT/morning_bulk_publish_watch.py" ]]; then
  cd "$ROOT"
  .venv/bin/python - <<PY 2>&1 | tail -n 40 || true
import json
from pathlib import Path
...
if [[ "$CACHE_OK" -eq 1 || "$DONE_OK" -eq 1 ]]; then
  ...
  log "ACTION: wait for morning bulk (06:00/07:30) or trigger admin 『① 一斉予想再実行』"
  # Show whether start/preflight ran
  section "cron start/preflight tails"
  tail -n 30 "$ROOT/logs/cron_race_day_start.log" 2>/dev/null || log "(no start log)"
  echo "----"
  tail -n 30 "$ROOT/logs/cron_race_day_preflight.log" 2>/dev/null || log "(no preflight log)"
  echo "----"
  tail -n 40 "$ROOT/logs/server_automation_debug.jsonl" 2>/dev/null || log "(no automation debug)"
fi
...
if [[ "$CACHE_OK" -eq 0 && "$DONE_OK" -eq 0 ]]; then
  ...
  log "verdict=PUBLISH_FAILED_OR_SKIPPED — 上記 force_publish / marks_baker 等のエラーを確認"
fi
log "DONE"
