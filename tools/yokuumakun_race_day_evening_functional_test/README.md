...
| チェック | 内容 | 自己修正 |
|---|---|---|
| start_schedule_armed | 05:00 start timer/cron + CRON_TZ | start bootstrap |
| stop_schedule_armed | 20:00 stop timer/cron | EOD bootstrap |
| evening_schedule_armed | 21:00 cron + CRON_TZ | evening bootstrap |
| publish_watch_timer | publish-watch.timer enabled | install / enable timer |
| preflight_evidence | 当日 preflight ログ | 対象外（証跡） |
| race_day_start_evidence | 当日 05:00 起動痕跡 | 対象外（翌朝は schedule 武装） |
...
| eod_snapshot_state | 当日アーカイブ or latest クリア | `clear_latest` / finalize |
...
curl -fsSL -o bootstrap_on_server.sh \
  "https://raw.githubusercontent.com/t-orz/keiba-mystery-viewer/cursor/race-day-timetable-guard-19c2/tools/yokuumakun_race_day_evening_functional_test/bootstrap_on_server.sh"
...
curl -fsSL -o "$ROOT/server_deployment/race_day_evening_functional_test.py" \
  "https://raw.githubusercontent.com/t-orz/keiba-mystery-viewer/cursor/race-day-timetable-guard-19c2/tools/yokuumakun_race_day_evening_functional_test/race_day_evening_functional_test.py"
