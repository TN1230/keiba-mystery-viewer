...
# [1] tunnel — embedded（同梱）。raw ブランチURLを優先（CDNキャッシュ回避）
EMBED_URL="https://raw.githubusercontent.com/t-orz/keiba-mystery-viewer/cursor/ssh-bore-endpoint-a29c/tools/yokuumakun_lan_apply_pending/bootstrap_tunnel_embedded.sh"
if [[ -n "$TUNNEL_REF" ]]; then
  EMBED_URL="https://cdn.jsdelivr.net/gh/t-orz/keiba-mystery-viewer@${TUNNEL_REF}/tools/yokuumakun_lan_apply_pending/bootstrap_tunnel_embedded.sh"
fi
