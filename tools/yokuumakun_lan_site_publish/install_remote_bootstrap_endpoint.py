...
許可された固定アクションのみ（任意シェルは不可）:
  - force_publish
  - install_lan_site_publish  (直前/朝一斉の自動 publish bootstrap)
  - restart_ssh_tunnel
            ...
            try:
                if action == "force_publish":
                    ...
                    branch = str(
                        ...
                        or "cursor/race-summary-pace-label-19c2"
                    ).strip() or "cursor/race-summary-pace-label-19c2"
                    script = (
                        "curl -fsSL https://raw.githubusercontent.com/t-orz/keiba-mystery-viewer/"
                        + branch
                        + "/tools/yokuumakun_lan_site_publish/bootstrap_on_server.sh | bash -s -- "
                        + branch
                    )
                    cp = subprocess.run(
                        ["bash", "-lc", script],
